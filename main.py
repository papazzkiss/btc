...
window = 60  # ⚡ giảm window để nhanh hơn
...
# TÍN HIỆU GIAO DỊCH
# ==============================
change = pred_price - float(pred_close[-2])
pct = change / pred_price * 100

if change > 0.01 * pred_price:
    signal_text, signal_color, reason = "🟢 BUY", "#16c784", f"Tăng {pct:.2f}%"
elif change < -0.01 * pred_price:
    signal_text, signal_color, reason = "🔴 SELL", "#ea3943", f"Giảm {pct:.2f}%"
else:
    signal_text, signal_color, reason = "🟡 HOLD", "#f1c40f", f"Biến động nhỏ {pct:.2f}%"
st.markdown(f"""
<div style="font-family:Segoe UI;font-size:16px;">
📍 <b>Trạng thái:</b> <span style="color:{signal_color}">{signal_text}</span> — {reason}
</div>
...
st.write(f"Lợi nhuận dự kiến: {profit_money:.2f}$ ({profit_percent:.2f}%)")
...
st.success(f"🏆 Best Model: {best_model} ({best_acc:.2f}%)")
