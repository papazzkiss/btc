...
st.sidebar.subheader("📩 Telegram Config")
tele_token = st.sidebar.text_input("Bot Token")
...
def train_model(X,y):
    ...
    model.fit(X,y,epochs=10,batch_size=32,verbose=0)  # giảm epoch cho nhanh
...
rf_mape = np.mean(np.abs((y_rf[split:].values - rf_pred) / y_rf[split:].values)) * 100
...
col4.metric("LSTM Acc", f"{accuracy:.2f}%")
col5.metric("RF RMSE", f"{rf_rmse:.0f}")
col6.metric("RF Acc", f"{rf_accuracy:.2f}%")
